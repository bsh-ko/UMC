...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    const fetchMovie = async (): Promise<void> => {
      setIsPending(true);
      try {
        const { data } = await axios.get<Movie>(`https://api.themoviedb.org/3/movie/${movieId}?language=en-EN`, {
          headers: {
      ...
      {!isPending && movie && (
        <div className="p-8">
          <div className="flex flex-col lg:flex-row gap-8">
            <img src={`https://image.tmdb.org/t/p/w300${movie.poster_path}`} className="w-80" />
